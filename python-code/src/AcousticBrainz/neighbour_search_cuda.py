...
def calc_quality(archive_d_qual, rows_, rows_curr_gpu):
    ...
    quality_sum_kernal[blocks_per_grid_quality, threads_per_block_quality](archive_d_qual, quality_sum, rows_curr_gpu)
    cuda.synchronize()
...
if __name__ == '__main__':
    ...
    if ROWS_MAX % CURRICULUM_SIZE != 0:
        CURRICULUM_LEVELS += 1
    A = np.array(df['hash_8192_bits'].values.tolist(), dtype=np.int32)
    ...
    print("Completed!\tWrapping up....")

    pd.DataFrame(data=stats, columns=['timestamp', 'curriculum_level', 'iteration', 'rows', 'quality_indicator_mean',
                                      'quality_indicator_std']) \
        .to_csv(
        config.CUDA_neighbour_search_stats_dir + '/' + str(datetime.now().strftime("%Y-%m-%d-T%H-%M-%S")) + '.csv',
        index=False)
    print("stored stats")
    ...
    df[['rec_MBID', 'row_id', 'neighbours', 'neighbour_dist']] \
        .to_parquet(f"{config.spark_output_dir}neighbours.parquet", index=False)
    print("stored neighbours")
